...
class CombineChecker:
    ...
    def check(self) -> bool:
        ...
        for _combo in self.combine:
            ...
            if _product_id and not self._check_product_id(_quantity, _product_id):
                return False

            if _product_option_id and not self._check_product_option_id(_quantity, _product_option_id):
                return False

        return True

    def _check_product_id(self, quantity: int, product_id: Optional[str], *args, **kwargs) -> bool:
        if not product_id:
            return True
    ...
    def _count_in_product_option(self, product_options: List[ProductOptionEntity], product_option_id: str) -> int:
        _count = 0
    ...
    def _count_in_subproducts(self, subproducts: List[SubproductEntity], product_option_id: str) -> int:
        _count = 0
    ...
    def _check_product_option_id(self, quantity: int, product_option_id: Optional[str], *args, **kwargs) -> bool:
        if not product_option_id:
            return True

        _total_occurs = 0
        for _line in self.order.lines:
            _total_occurs += self._count_in_product_option(_line.product_options, product_option_id)
            _total_occurs += self._count_in_subproducts(_line.subproducts, product_option_id)
